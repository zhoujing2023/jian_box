...
impl AppLoader {
    ...
    /// # Examples
    /// ```
    /// use jian_box::{AppLoader, Config};
    /// let config = Config::load().unwrap();
    /// let apps = AppLoader::load(&config);
    /// ```
    pub fn load(config: &Config) -> Apps {
        let desktop_paths: Vec<PathBuf> = vec![
            PathBuf::from("/usr/share/applications"),
            PathBuf::from("/var/lib/snapd/desktop/applications"),
            config.home_dir.join(".local/share/applications"),
        ];
        // 目录条目集合
        ...
        let (env_name_prefix, env_comment_prefix) = (
            format!("Name[{}]=", &config.language),
            format!("Comment[{}]=", &config.language),
        );
        ...
        // 读取分数
        let app_configs = AppUsage::load(&config);
        if let Some(app_configs) = app_configs {
            for app in &mut apps {
                *app.score.borrow_mut() = app_configs
                    .scores
    ...
    ///
    /// ```no_run
    /// use jian_box::AppLoader;
    /// use std::path::PathBuf;
    /// let dir_entry_list = AppLoader::parse_dir_entry(vec![
    ///         PathBuf::from("/usr/share/applications"),
    ...
    /// let content = AppLoader::check_desktop(content);
    /// ```
